...
# hacknheal Help Context
def helper():
        print bcolors.OKBLUE+"Information:"+bcolors.ENDC
        print "------------"
        print "\t./hacknheal.py example.com: Scans the domain example.com"
        print "\t./hacknheal.py --update   : Updates the scanner to the latest version."
        print "\t./hacknheal.py --help     : Displays this help context."
        print bcolors.OKBLUE+"Interactive:"+bcolors.ENDC
        print "------------"
        print "\tCtrl+C: Skips current test."
        print "\tCtrl+Z: Quits hacknheal."
	...
	print bcolors.WARNING
        print("""\
                ...
                |_| |_|\__,_|\___|_|\_\_\ \/ |_| |_|\___|\__,_|_|
                 ...
                 [Developed by team hacknheal from Axis Colleges]

                Team members are Satyendra, Suraj, Ashutosh, Surya                                             
                                                                       
                   """+bcolors.ENDC+"""(A PIPELINE based Web Vulnerability Scanner)
                            """)
        print bcolors.ENDC
...
class Spinner:
    ...
    def spinner_task(self):
        try:
            ...
            #clear()
            print "\n\t"+ bcolors.BG_ERR_TXT+"hacknheal received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC
            sys.exit(1)
...
if len(sys.argv) == 1 :
    ...
    if target == '--update' or target == '-u' or target == '--u':
    	logo()
        print "hacknheal is updating....Please wait.\n"
        spinner.start()
        ...
        if rs_internet_availability == 0:
            ...
            spinner.stop()
            sys.exit(1)
        cmd = 'sha1sum hacknheal.py | grep .... | cut -c 1-40'
        ...
        if oldversion_hash == newversion_hash :
            clear()
            print "\t"+ bcolors.OKBLUE +"You already have the latest version of hacknheal." + bcolors.ENDC
        else:
            clear()
            print "\t"+ bcolors.OKGREEN +"hacknheal successfully updated to the latest version." +bcolors.ENDC
        spinner.stop()
        sys.exit(1)
        ...
        while (rs_avail_tools < len(tools_precheck)):
			...
			try:
				...
				print "\t"+bcolors.BG_ERR_TXT+"hacknheal was terminated abruptly..."+bcolors.ENDC
				sys.exit(1)
			if "not found" in val:
        ...
        if unavail_tools == 0:
        	print "\t"+bcolors.OKGREEN+"All Scanning Tools are available. All vulnerability checks will be performed by hacknheal."+bcolors.ENDC
        else:
        	print "\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable. hacknheal can still perform tests by excluding these tools from the tests. Please install these tools to fully utilize the functionality of hacknheal."+bcolors.ENDC
		...
		rs_skipped_checks = rs_skipped_checks + 1
            ...
            if runTest == 1:
                    ...
                    clear()
                    print "\t"+bcolors.WARNING + "Test Skipped. Performing Next. Press Ctrl+Z to Quit hacknheal." + bcolors.ENDC
                    rs_skipped_checks = rs_skipped_checks + 1
        ...
        if len(rs_vul_list)==0:
        	...
        	with open("RS-Vulnerability-Report", "a") as report:
        		while(rs_vul < len(rs_vul_list)):
	       		...
	       		print "\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`RS-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory hacknheal resides."
